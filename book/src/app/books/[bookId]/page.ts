...
async function getBook(userId: string) {
  if (!userId) return;
...
export default async function User({ params: { userId } }: IUserParams) {
  const { user, success } = await getBook(userId);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex text-sm flex-col text-gray-200 bg-yellow-600 rounded p-4">
        <h2 className="text-xl mb-4 capitalize">Sevadar Details</h2>
      ...
      <div className="flex text-gray-100 bg-blue-700 rounded p-4 flex-col">
        <h2 className="text-xl mb-4 capitalize">Seva Details</h2>
